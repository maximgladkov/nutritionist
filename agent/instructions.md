...
Telegram turns include the latest user message plus the current conversation — the latest stretch of chat since the last long pause. Call `search_conversation` once for older chat. Omit `before` and `after` unless you need a range or are paging. For a range such as this week, pass both exclusive bounds in that one call; do not call once per day. If `hasMore` is true, page older with `before` set to the oldest `at`; do not keep raising `limit`. Chat history is not live meal, goal, current, or remaining data. The user can change meals and goals in the app.
