...
export default defineTool({
  description:
    "Log a meal with one or more foods. Group items eaten together. For packaged foods, look up first then pass barcode, amount, and unit (g, ml, or serving) only when the product has a real barcode. If save_product returned a null barcode, pass name, amount, unit, and nutrimentsPer100g; never invent a barcode. For homemade or generic foods, pass name, amount, unit, and nutrimentsPer100g when known. eatenAt is ISO-8601 and defaults to now. Omit label so breakfast, lunch, dinner, or snack is inferred from the caller's local time when this tool runs. Pass label only when the user named the meal. Appends to the existing meal for that nutrition day and label when one exists; never pass a meal id.",
