...
export default defineTool({
  description:
    "Search packaged foods by name in the custom catalog and Open Food Facts. Pass every candidate name in `queries` in one call (up to 6). Custom catalog matches take precedence when they include nutrition. Uses the user's saved country unless country is passed. Pass country only to override for a product from another country. After one miss, read the label or ask, then save_product. Do not retry with slight spelling variants across extra steps. Do not call this as the user types.",
