...
export default defineTool({
  description:
    "Required when stating what the caller ate. Do not reconstruct meals from chat. Lists meals for nutrition dates in a range, grouped into breakfast, lunch, dinner, and snack. Omit from and to to use today's nutrition day (04:00 to 04:00 the next morning in the saved timezone). from and to are inclusive YYYY-MM-DD nutrition dates. If only one is passed, that single day is used. If they are reversed, they are swapped. Do not pass a meal type; the payload includes currentLabel from the local clock. For a single day it also returns goals, current, and remaining. Do not use those numbers from chat.",
